...
class Audio:
    ...
    async def play(self, ctx, *, url_or_search_terms=None):
        """Plays a song / Defaults to queue."""
        ...
        songlist = None
        if self._match_sc_playlist(url):
            welp1 = await self.bot.say("Enumerating soundcloud playlist... This could take a few moments")
            songlist = await self._parse_sc_playlist(url)
        elif self._match_yt_playlist(url):
            welp1 = await self.bot.say("Enumerating youtube playlist... This could take a few moments")
        ...
        if songlist:
            ...
            welp2 = await self.bot.say("Enumerated {} tracks adding them to the playlist.... :writing_hand::skin-tone-3:".format(len(songlist)))
            await self.bot.delete_message(welp1)
            await asyncio.sleep(5)
            await self.bot.delete_message(welp2)
            ...
            await self.bot.say("Added your song to the playlist. :writing_hand::skin-tone-3:")
    ...
    async def stop(self, ctx):
        ...
        if self.is_playing(server):
            if ctx.message.author.voice_channel == server.me.voice_channel:
                ...
                await self.bot.say('Nothing playing, nothing to stop.')
    ...
    async def _embed_np(self, message, server:discord.Server=None, channel:discord.Channel=None, author:discord.Member=None):
        ...
        if song:
            hmk = discord.Embed(description="Connected to {}.".format(server.me.voice_channel), colour=discord.Colour.blue())
            hmk.add_field(name="Now playing...", value="Gathering information....")
            hmk.set_footer(text=author.display_name+" | Song information.", icon_url=author.avatar_url)
            hmk2 = await self.bot.send_message(channel, embed=hmk)
            ...
            if hasattr(song, 'duration'):
                ...
                dur = None
            embed = discord.Embed(title="Connected to {}.".format(server.me.voice_channel), description="**Now playing**", colour=discord.Colour.blue())
            embed.add_field(name="Title", value="{}".format(song.title))
            embed.add_field(name="Duration", value="{}".format(dur))
            if song.webpage_url:
                embed.add_field(name="Url", value="{}".format(song.webpage_url))
            if song.thumbnail:
                embed.set_image(url=song.thumbnail)
            embed.set_footer(text=author.display_name+" | Song information.", icon_url=author.avatar_url)
            #embed.set_thumbnail(url=song.thumbnail)
            await asyncio.sleep(2)
            okay = await self.bot.edit_message(hmk2, embed=embed)
            await asyncio.sleep(10)
            await self.bot.delete_message(okay)
